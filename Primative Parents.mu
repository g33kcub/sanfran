...
@conformat #32=[u(conformat`extra)][if(words([mplayers(%!,%#)] [lvthings(%!)]),[line()])][if(words([mplayers(%!,%#)]),udefault(PLAYERS`[u(system`players)],u(PLAYERS`DEFAULT)))][if(words([lvthings(%!)]),udefault(THINGS`[u(system`things)],u(Things`DEFAULT)))]

&players`default #32=%R[ansi([gameconfig(columns)],Players)]:%R[iter([sortby(sort`name,[mplayers(%!,%#)])],[u(players`default`fmt,##,%#)],%B,%R)]
&players`default`fmt #32=[align(>3 35 [lmath(sub,[u(width,%1)] 3 1 35 1 3 1)] >3,[statustag(%0)],[moniker(%0)][if(cor(ispowered(%1),controls(%1,%0)),%(%0%))],[if(isstaff(%0),[staffbit(%0)],[sdesc(%0)])],[hideidle(%0)])]
